  ...
  <div class="content-container">
    ...
    <div class="addnew">
      ...
      <div class="killprice">
        ...
        <el-input v-model="goodsMsg.point" placeholder="输入数量" class="money"></el-input>
  ...
  methods: {
    ...
    dialogAllData() {
      this.formSearch.goodsCategoryId =
        ...
        this.selectedCategoryMenu; // 商品类目id
    ...
    getSingleData() {
      // 获取当前页面接收到的id
      this.goodsActivityId = this.$route.params.id;
      // 存放时间
      const timeList = [];
      if (this.goodsActivityId) {
        fastestSingleCheck(this.goodsActivityId).then(res => {
          ...
          // 赋值商品名称
          this.goodsMsg.goodsname = this.singleData.name;
          // 赋值教师名称
          this.goodsMsg.teacher = this.singleData.teacherNames;
          // 赋值商品原价
          this.goodsMsg.price = this.singleData.price;
          // 赋值可用库存
          this.goodsMsg.usestore = this.singleData.goods.stockCount;
          // 活动库存
          this.goodsMsg.repertory = this.singleData.repertory;
          // 赋值积分数量
          this.goodsMsg.point = this.singleData.goods.point;
          // 加入开始和结束时间
    ...
    async submitMsg() {
      ...
      if (
        ...
        Number(this.goodsMsg.repertory) > Number(this.goodsMsg.usestore)
      ) {
        this.$message.error("活动总库存不能高于可用库存!");
        res = false;
