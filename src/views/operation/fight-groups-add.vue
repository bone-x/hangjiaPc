<template>
  <div class="content-container">
    ...
    <section class="fight-groups-add">
      <h4>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-row>
              ...
              <el-col :span="12" center>
                <el-button @click.native="clearSelect()">清空</el-button>
              </el-col>
            </el-row>
          </el-col>
        </el-row>
...
import { createFightGroup, getDialogGoodsList } from "@/api/operation";
...
export default {
  ...
  methods: {
    // 弹窗清空
    clearSelect() {
      this.selectGoodsWindow.selectedCategory = [];
      this.selectGoodsWindow.name = '';
    },
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          createFightGroup(options).then(res => {
            if (res.code === 200) {
    ...
    onCategoryChange(valArr, index) {
      ...
      console.log(this.ruleForm.effectValueArr)
    },
    ...
    onSelectLecturerCheckBox(selection, isSelect, isAll) {
      // 过滤
      const filterByPriceArr = [];
      selection.forEach(i => {
        if (i.price > 0) filterByPriceArr.push(i);
      });
      selection = filterByPriceArr.concat([]);
