          ...
          >
            <div class="slideImg">
              ...
              <div class="atention">
                ...
                <p style="color: red; margin: 0px; font-size: 12px">图片尺寸： 宽750~1080 px，高1000~1600 px</p>
              </div>
            </div>
          </el-form-item>
        </el-col>
          ...
          >
            <div class="slideImg">
              ...
              <div class="atention">
                ...
                <p style="color: red; margin: 0px; font-size: 12px">图片尺寸： 600*800px</p>
              </div>
            </div>
          </el-form-item>
        </el-col>
...
import { getDialogGoodsList, getAppDetails } from '@/api/operation';
...
export default {
  ...
  created() {
    ...
    this.getGoodsList();
    this.getAppAdverDetails();
  },
  methods: {
    // 获取详情
    async getAppAdverDetails() {
      // console.log('e', this.$route.params.id);
      const options = {
        id: this.$route.params.id,
      };
      const res = await getAppDetails(options);
      if (res.code !== 200) return Promise.reject();
      this.ruleForm.position = res.result.position;
      this.ruleForm.skipType = res.result.skipType;
      if (res.result.skipType === 2) {
        this.adverNameList.map(item => {
          if (Number(res.result.name) === item.value) {
            this.ruleForm.name = item.label;
          }
      ...
      this.ruleForm.image = decodeURIComponent(res.result.image);
      this.ruleForm.timeLimit = [
        res.result.showStartTime,
        res.result.showEndTime,
      ];
      this.ruleForm.showStartTime = res.result.showStartTime;
      this.ruleForm.showEndTime = res.result.showEndTime;
      this.ruleForm.displayOnceTime = res.result.displayOnceTime;
      this.ruleForm.displayDayTimes = res.result.displayDayTimes;
      this.ruleForm.status = res.result.status;
      this.ruleForm.bandGoodsId = res.result.bandGoodsId;
    ...
    async getGoodsList(pageNum = 1) {
      ...
      // this.dialog.pageSize = res.result.pageSize;
    ...
    onGoodsWindowConfirm() {
      if (this.dialogMsg.selectedGoods) {
        ...
        this.ruleForm.skipLink = this.dialogMsg.selectedGoods.carouselUrl;
