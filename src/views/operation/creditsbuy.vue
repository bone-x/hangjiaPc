    ...
    <div class="creditsbuy">
      ...
      <div class="pageheard">
        <el-form :inline="true" :model="formInline" class="flex-center">
          ...
          <el-button
            v-permission="['operate:integral:save']"
            type="primary"
            class="addnew"
            @click.native="goNew"
          >新建活动</el-button>
      ...
      <div class="tables">
        <el-table :data="fastData" border style="width: 100%">
          <el-table-column :show-overflow-tooltip="true" prop="goods.name" label="积分商品" width="180"></el-table-column>
          ...
          <el-table-column prop="point" label="兑换积分" min-width="120"></el-table-column>
          <el-table-column prop="goods.price" label="商品价（元）" min-width="120"></el-table-column>
          <el-table-column prop="updateName" label="修改人" min-width="120"></el-table-column>
          <el-table-column prop="overtime" label="积分活动时间" min-width="220">
          ...
          <el-table-column fixed="right" label="操作" min-width="250">
            <template slot-scope="scope">
              <router-link
                v-permission="['operate:integral:update']"
                v-if="scope.row.$showBtnEdit"
                :to="{ name: 'OperationManageCreditsbuyEdit', params: { id: scope.row.id } }"
              >
              ...
              <el-button
                v-permission="['operate:integral:delete']"
              ...
              <el-button
                v-permission="['operate:integral:ud']"
      ...
      <el-dialog :visible.sync="centerDialogVisible" title="查看详情" width="900px" left>
        <el-form :inline="true" :model="goodDetail" class="flex-center">
          ...
          <el-button
            v-permission="['operate:integral:export']"
            class="alldata"
...
export default {
  ...
  methods: {
    onRouteChange(to) {
      if (to.name === "OperationManageCreditsbuy") {
        this.getFastestBuyPageData();
      }
    },
    ...
    // 跳转新建页面
    goNew() {
      this.$router.push("credits-buy/add");
    },
    ...
    dialogHandleSizeChange(val) {
      this.goodDetail.pageSize = val;
    },
    dialogHandleCurrentChange(val) {
      this.goodDetail.pageNum = val;
      this.searchData();
    },
