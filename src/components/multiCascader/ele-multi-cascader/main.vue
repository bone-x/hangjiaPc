        ...
        <ul class="el-cascader-menu cascader-menu" v-for="(cas, index) in casTree" :key="index">
          ...
          >
            <el-checkbox
              ...
              @change="checked => { onSelectAll(cas, checked) }"
            ></el-checkbox>
            <el-checkbox
              v-else
              :disabled="item.disabled"
...
export default {
  ...
  methods: {
    onSelectAll(items = [], checked) {
      items.forEach(item => {
        this.checkedChange(item, checked);
      });
    },
    initOpts() {
    ...
    recursiveOpt(nodeArr, parent) {
      ...
      nodeArr.forEach(node => {
        ...
        if (
          this.value.some(
            val => val == this.getLevel(node, "value", this.outputLevelValue)
          )
        ) {
          node.checked = true;
        }
    ...
    markParentChecked(node) {
      node.indeterminate = false;
      const that = this;
      function checkTopCheckAll() {
        if (that.casTree && that.casTree[0] && that.casTree[0][0]) {
          let isCheckedAll = true;
          that.casTree[0].some(item => {
            if (!item.checked && item.label !== "全选") {
              isCheckedAll = false;
              return true;
            }
          });
          that.casTree[0][0].checked = isCheckedAll;
        }
      }

      function loop(node) {
        let checkCount = 0;
        let childIndeterminate = node.children.some(
          child => child.indeterminate
        );
        node.children.map(child => {
          if (child.checked && child.label !== "全选") {
            checkCount++;
          }
        });
        // 子节点全部被选中
        if (checkCount >= node.children.length - 1) {
          node.children[0].checked = true;
          node.checked = true;
          node.indeterminate = false;
        } else {
          if (that.casTree && that.casTree[0] && that.casTree[0][0]) {
            that.casTree[0][0].checked = false;
          }
      ...
      if (node) {
        if (node.parent) {
          if (this.selectChildren) loop(node.parent);
        } else {
          // 顶级
          checkTopCheckAll();
        }
    ...
    markParentHasCheckChild(node) {
      ...
      function loop(node) {
        let checkCount = 0;
        let childHasCheckedChild = node.children.some(
          child => child.hasCheckedChild
        );
        node.children.map(child => {
          if (child.checked) {
            checkCount++;
        ...
        if (Array.isArray(data)) {
          data.map(item => {
            if (item.checked && item.label !== "全选") {
              ...
              vm.selectedLabels.push(
                vm.getLevel(item, "label", vm.showAllLevels)
              );
              vm.selectedValues.push(
                vm.getLevel(item, "value", vm.outputLevelValue)
              );
            }
    ...
    // 一旦有一个选项改为false, 递归设置父级全选为false
    clearSelectAll(item) {
      this.casTree[0][0].checked = false;
      if (item) {
        if (item.parent) {
          item.parent.children[0].checked = false;
          this.clearSelectAll(item.parent);
        }
      }
    },
    ...
    isSelectedAll(item) {
      ...
      if (item) {
        if (item.parent) {
          for (let i = 1; i < item.parent.children.length; i++) {
            const li = item.parent.children[i];
            if (!li.checked) {
              isChecked = false;
              break;
            }
          }
          item.parent.children[0].checked = isChecked;
          this.isSelectedAll(item.parent);
        } else {
          // 判定为顶级ul
          for (let i = 1; i < this.casTree[0].length; i++) {
            const li = this.casTree[0][i];
            if (!li.checked) {
              isChecked = false;
              break;
            }
          }
          this.casTree[0][0].checked = isChecked;
        }
      }
    },
    ...
    syncData() {
      ...
      this.$emit("change", this.selectedValues, this.selectedItems, this.casTree[0]);
    },
    // 展开下一级
    ...
    setPopperWidth() {
      ...
      document.getElementsByClassName(this.popperClass)[0].style.width =
        width + "px";
    },
